...
class IntArray:
    ...
    def insert(self, index: int, val: int) -> None:
        ...
        if not isinstance(val, int) or not self._min_val <= val <= self._max_val:
            raise TypeError('Value must be an integer between {self._min_val} and {self._max_val}')
        elif not (0 <= index < self._size + 1):
            raise IndexError('Index is out of bounds!')


        # Update array's size0
        self._size += 1
        ...
        # Copy the old reserved memory area (if there was one)
        if self._resmem:
            new_resmem.copy(self._resmem, index*self._bytes_per_element)

        new_resmem.copy(source_index=index * self._bytes_per_element, destination_index=(index+1) * self._bytes_per_element, count=(self._size - index -1)* self._bytes_per_element, mem_source=self._resmem)
        self._resmem = new_resmem


        # Store the new value at the index
    ...
    def remove(self, index: int) -> int:
        ...
        """
        Remove an element at the given index in an array
        """
        if not isinstance(index, int) or not self._min_val <= index <= self._max_val:
            raise TypeError('Value must be an integer between {self._min_val} and {self._max_val}')
        ...
        if self._size > 0:
            ...
            # And copy the old memory area (except last element)
            new_resmem.copy(self._resmem, index* self._bytes_per_element)
            new_resmem.copy(source_index=(index+1)* self._bytes_per_element,
                            destination_index=index * self._bytes_per_element,
                            count=(self._size - index) * self._bytes_per_element, mem_source=self._resmem)
